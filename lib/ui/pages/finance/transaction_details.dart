...
class _PageState extends BaseLoadRefreshDataWidgetState<TransactionDetailsPage, TransactionRecordDetailsViewModel> {
  @override
  ...
  Widget buildBodyWidget(BuildContext context) {
        ...
        Expanded(
            child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Image.asset(ImageHelper.wrapAssets('icon_success.png'), width: 66, color: ThemeColors.primaryFgColor
                        // fit: BoxFit.contain,X
                        ),
                    Text(
                      "USDT",
                      style: Theme.of(context).textTheme.headline4.copyWith(fontWeight: FontWeight.bold),
                    ).padding(EdgeInsets.symmetric(vertical: ThemeDimens.pageVerticalMargin * 3)),
                    Text("${widget.data.out ? "-" : "+"}84.9",
                        style: Theme.of(context).textTheme.headline4.copyWith(
                              fontWeight: FontWeight.bold,
                              fontSize: 30,
                              color: widget.data.out ? ThemeColors.red : ThemeColors.green,
                            )),
                    Column(
    ...
    ).padding(EdgeInsets.only(left: ThemeDimens.pageLRMargin, right: ThemeDimens.pageLRMargin, top: ThemeDimens.pageVerticalMargin * 1.5, bottom: ThemeDimens.pageBottomMargin));
  }
  ...
  buildLabelAndContent() {
    ...
    for (var label in labels) {
      ...
      ]).padding(EdgeInsets.symmetric(vertical: ThemeDimens.pageVerticalMargin)).border(Border(bottom: BorderSide(color: ThemeColors.labelLightColor, width: 1, style: BorderStyle.solid))));
    }
    return widgets;
  }
}
